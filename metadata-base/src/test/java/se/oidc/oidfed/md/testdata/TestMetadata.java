  ...
  static {
    try {
      ...
      federationEntityMetadata = FederationEntityMetadata.builder()
          ...
          .endpointAuthSigningAlgValuesSupported(List.of("RS256", "ES256", "PS256"))
          .build().toJsonObject();
      federationEntityMetadataJson =
          MetadataUtils.OBJECT_MAPPER.writerWithDefaultPrettyPrinter().writeValueAsString(federationEntityMetadata);

      opMetadata_claim123 = OpMetadata.builder()
          .scopesSupported(List.of("openid"))
          .claimsSupported(List.of("claim1", "claim2", "claim3"))
          .build().toJsonObject();
      opMetadata_claim123Json =
          MetadataUtils.OBJECT_MAPPER.writerWithDefaultPrettyPrinter().writeValueAsString(opMetadata_claim123);

      opMetadata_claim12 = OpMetadata.builder()
          .scopesSupported(List.of("openid"))
          .claimsSupported(List.of("claim1", "claim2"))
          .build().toJsonObject();
      opMetadata_claim12Json =
          MetadataUtils.OBJECT_MAPPER.writerWithDefaultPrettyPrinter().writeValueAsString(opMetadata_claim12);

      rpMetadata_code = RelyingPartyMetadata.builder()
          .responseTypes(List.of("code"))
          .build().toJsonObject();
      rpMetadata_codeJson =
          MetadataUtils.OBJECT_MAPPER.writerWithDefaultPrettyPrinter().writeValueAsString(rpMetadata_code);
